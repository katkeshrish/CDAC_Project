module Async_Tx_FIFO #(
    ...
    always_ff @(posedge ACLK or negedge rst_n) begin
        if (!ARESETN) begin
            ...
            wr_ptr_gry <= ((wr_ptr_bin + 1) >> 1) ^ (wr_ptr_bin + 1); // we are adding 1 because we want gry and bin pointer to be in sync
    ...
    // Full: MSB and MSB-1 are inverted, remaining bits same
        assign full  = (wr_ptr_gry == {~rd_ptr_sync2[ADDR_WIDTH:ADDR_WIDTH-1], rd_ptr_sync2[ADDR_WIDTH-2:0]}); //uses reflective property of gray code to find out FULL flag

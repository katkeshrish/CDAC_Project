...
// 4. Instantiate FIFOs (Tx and SS) with Asynchronous Support
    Async_Tx_FIFO tx_fifo_inst (
        .ACLK(ACLK),          // AXI Clock domain [cite: 35]
        .SCLK(sclk_int),      // SPI Clock domain [cite: 35]
        .ARESETN(ARESETN),    // Unified Reset [cite: 35]
        .data_in(tx_data_bus),// From AXI Slave [cite: 35, 71]
        .wr_en(wr_en),        // From AXI Slave [cite: 71, 72]
        .rd_en(rd_en),        // From SPI Master [cite: 52, 72]
    ...
    Async_SS_FIFO ss_fifo_inst (
        .ACLK(ACLK),          // AXI Clock domain [cite: 18]
        .SCLK(sclk_int),      // SPI Clock domain [cite: 18]
        .ARESETN(ARESETN),    // Unified Reset [cite: 18]
        .data_in(ss_data_bus),// From AXI Slave [cite: 18, 71]
        .wr_en(wr_en),        // From AXI Slave [cite: 71, 74]
        .rd_en(rd_en),        // From SPI Master [cite: 52, 74]
